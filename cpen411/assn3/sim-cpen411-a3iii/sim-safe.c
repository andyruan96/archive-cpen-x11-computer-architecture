...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_formula(sdb, "sim_inst_rate",
	...
	stat_reg_counter(sdb, "sim_num_mispredict", 
		"total number of conditional branches executed", &g_total_mispredictions, g_total_mispredictions, NULL);
...
#include "machine.def"
	...
	// iii. 1 bit predictor; 1 bit branch history
	if ( MD_OP_FLAGS(op) & F_COND ) {
		g_total_cond_branches++;
		unsigned index = (regs.regs_PC >> 3) & ((1<<17)-1);	// log2(262144 / 2) = 17
		assert( index < 262144 / 2 );
		int prediction = bpred_pht[gbhr][index];
		int actual_outcome = (regs.regs_NPC != (regs.regs_PC + sizeof(md_inst_t)));
		if( prediction != actual_outcome ) g_total_mispredictions++;

		bpred_pht[gbhr][index] = actual_outcome;
		gbhr = actual_outcome;
	}
