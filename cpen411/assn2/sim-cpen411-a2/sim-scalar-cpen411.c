...
#if FORWARDING==1		
			// RAW hazard, must stall if the write is a load that has not completed the memory access stage
			if( (MD_OP_FLAGS(pI->src[i]->op) & F_MEM) && 
				!(pI->src[i]->status == MEMORY_STAGE_COMPLETED ||
				pI->src[i]->status == DONE) ){
					pI->stalled = 1;
					return;
			}
			// Branch resolution cannot happen at the same time as data is first available
			else if( MD_OP_FLAGS(op) & F_COND ){
				if( pI->src[i]->status == EXECUTED || ((MD_OP_FLAGS(pI->src[i]->op) & F_LOAD) && (pI->src[i]->status == MEMORY_STAGE_COMPLETED)) ){
					pI->stalled = 1;
					return;
				}
			}
